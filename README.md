...
- Per-consumer bearer token auth and rate limiting, same shape as `tools/`; `helmet` for HTTP header security
...
- `express-rate-limit` - per-consumer request rate limiting
